...
export default function Indicator({
  ...
  for (let i = 0; i < totalPage; i += 1) {
    ...
    if (renderIndicator) {
      indicator = renderIndicator({ selected, index: i });
    } else {
      indicator = <Dot key={`index${i}`} selected={selected} />;
    }
...
function Dot({ selected }: { selected: boolean }) {
  return <View style={selected ? styles.dotSelectedStyle : styles.dotStyle} />;
}
